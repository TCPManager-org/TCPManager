...
public class MealService {
  ...
  public static MealResponse mapToMealResponse(Meal meal) {
    ...
    for (var mealIngredient : meal.getMealIngredients()) {
      ...
      calories += ingredient.getCalories() * mealIngredient.getWeight();
      carbs = carbs.add(
          ingredient.getCarbs().multiply(BigDecimal.valueOf(mealIngredient.getWeight())));
      fats = fats.add(
          ingredient.getFats().multiply(BigDecimal.valueOf(mealIngredient.getWeight())));
      proteins = proteins.add(
          ingredient.getProteins().multiply(BigDecimal.valueOf(mealIngredient.getWeight())));
      ingredients.put(ingredient.getId(), ingredient.getName());
      weight += mealIngredient.getWeight();
    }
    calories = calories / weight;
    carbs = carbs.divide(BigDecimal.valueOf(weight), RoundingMode.HALF_EVEN);
    fats = fats.divide(BigDecimal.valueOf(weight), RoundingMode.HALF_EVEN);
    proteins = proteins.divide(BigDecimal.valueOf(weight), RoundingMode.HALF_EVEN);
  ...
  public List<MealResponse> getMeals(Integer minIngredients, Integer maxIngredients,
    ...
    var meals = mealRepository.findAll().stream()
        .filter(meal -> isMealAvailableToUser(username, meal)).toList();

    if (minIngredients != null) {
      meals = meals.stream().filter(meal -> meal.getMealIngredients().size() >= minIngredients)
          .toList();
    }
    if (maxIngredients != null) {
      meals = meals.stream().filter(meal -> meal.getMealIngredients().size() <= maxIngredients)
          .toList();
    }
